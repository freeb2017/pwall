...
class INIParser
{
	...
	private static function _processSection($section, array $contents)
	{
		...
		// section extends another section
			...
			$sections_deli = explode('/', $section);
			if(count($sections_deli) == 2) {
			list($ext_source, $ext_target) = explode('/', $section);
			$ext_target = trim($ext_target);
			$ext_source = trim($ext_source);

			// check if the extended section exists
			if (!isset(self::$_result[$ext_source])) {
				self::$_result[$ext_source] = array();
			}

			// process section contents
			if( !isset( self::$_result[$ext_source][$ext_target] ) )
				self::$_result[$ext_source][$ext_target] = self::_processSectionContents($contents);
			else{

				$stored_value = self::$_result[$ext_source][$ext_target];
				self::$_result[$ext_source][$ext_target] = self::_processSectionContents($contents);

				foreach( $stored_value as $stored_key => $stored_data ){

					self::$_result[$ext_source][$ext_target][$stored_key] = $stored_data;
				}
			}
			...
			//self::$_result[$ext_target] = self::_arrayMergeRecursive(self::$_result[$ext_source], self::$_result[$ext_target]);
			...
			} elseif (count($sections_deli) == 3) {
				list($parent,$ext_source, $ext_target) = explode('/', $section);
				$ext_target = trim($ext_target);
				$ext_source = trim($ext_source);
				$parent = trim($parent);
				if (!isset(self::$_result[$parent])) {
					self::$_result[$parent] = array();
					self::$_result[$parent][$ext_source] = array();
				}

				if (!isset(self::$_result[$parent][$ext_source])) {
					self::$_result[$parent][$ext_source] = array();
				}

				self::$_result[$parent][$ext_source][$ext_target] = self::_processSectionContents($contents);


			}elseif (count($sections_deli) == 4) {
				list($parent,$ext_source, $ext_target, $shard_target) = explode('/', $section);
				$ext_target = trim($ext_target);
				$ext_source = trim($ext_source);
				$shard_target = trim($shard_target);
				$parent = trim($parent);
				if (!isset(self::$_result[$parent])) {
					self::$_result[$parent] = array();
					self::$_result[$parent][$ext_source] = array();
				...
				if(!isset(self::$_result[$parent][$ext_source][$ext_target])){
					self::$_result[$parent][$ext_source][$ext_target] = array();
				}

				self::$_result[$parent][$ext_source][$ext_target][$shard_target] = self::_processSectionContents($contents);
